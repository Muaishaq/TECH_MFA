...
const { sendBatchApprovalEmail, sendBatchRejectionEmail } = require('../services/email.service');

// @desc    Get all batches (open by default, all if ?all=true)
// @route   GET /api/batches
// @access  Public
const getBatches = asyncHandler(async (req, res) => {
  const { academy, all } = req.query;

  const where = all === 'true' ? {} : { status: 'open' };
...
const approveEnrollment = asyncHandler(async (req, res) => {
  ...
  const student = await prisma.user.findUnique({ where: { id: enrollment.student_id } });
  if (student && enrollment.batch) {
    sendBatchApprovalEmail(student, enrollment.batch);
  }
...
const rejectEnrollment = asyncHandler(async (req, res) => {
  ...
  const student = await prisma.user.findUnique({ where: { id: enrollment.student_id } });
  if (student && enrollment.batch) {
    sendBatchRejectionEmail(student, enrollment.batch);
  }
